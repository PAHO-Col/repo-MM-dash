...
class reports_Core {
	 ...
	 * @mr_k ::: se adicionara en el registro de un reporte el id del crue.
	 *
	 */
	public static function save_report($post, $incident, $location_id)
	{
		
		Kohana::log('debug', ":::::::: HELPER::: REPORTS :::::FUNCION SAVE_REPORT");
		$sql = "SELECT crue_id FROM crues_ips WHERE Id='%s'";
		...
		{	
			...
			//mr_k  ::: se adiciona la consulta del campo crue_id del usuario y se a
			$incident->crue_id = Auth::instance()->get_user()->crue_id;			
			$incidentcrue_id = Auth::instance()->get_user()->crue_id;
			Kohana::log('debug', "::::::::HELPER ::: consulta id crue_di ::::::::");
			Kohana::log('debug', $incidentcrue_id);
			Kohana::log('debug', $incident->crue_id);
			
			
			$sql = "SELECT crue_id FROM crues_ips WHERE Id='$incidentcrue_id'";
			//echo "<br>LA CONSULTA ES: "+$sql;
			Kohana::log('debug', ":::::::: LA CONSULTA sql  ::::::::");
			Kohana::log('debug', $sql);
			$incidentcrue_idx = $db->query($sql);			
			//print_r($incidentcrue_idx);
			foreach ($incidentcrue_idx as $incidentcrue_idxx)
				{				

					$elcrue_id = $incidentcrue_idxx->crue_id;
					Kohana::log('debug', ":::::::: LA CONSULTA elcrue_id  ::::::::");
					Kohana::log('debug', $elcrue_id);
				}
			//	print_r("SIGUE EL elcrue_id");
			//	print_r($elcrue_id);
			//$incident->crue_id = $db->query('SELECT crue_id FROM crues_ips WHERE Id="'+$incidentcrue_id+'"');			
			/*$incident->crue_id = ORM::factory('ips')			
			->where('Id',$incidentcrue_id)
			->find_all();*/
			if($elcrue_id !=""){
				$incident->crue_id=$elcrue_id;
			}
			
			Kohana::log('debug', ":::::::: consulta id crue_di ::::::::");
			Kohana::log('debug', $elcrue_id);
			//Kohana::log('debug', ":::::::: consulta id crue_di ::::::::");
			//Kohana::log('debug', $incidentcrue_idx);
		...
		// mr_k::: Verification status:  Only set if user has permission
		if (isset($post->incident_action) AND Auth::instance()->has_permission('reports_acciones'))
		{
			$incident->incident_action = $post->incident_action;
		}
	...
	public static function verify_approve($incident)
	{
		...
		elseif ($incident->incident_action == 1) // Nuevo estado para incident_action
		{
			$verify->verified_status = '4'; // Acción tomada
		}
		else
		{
		...
		//die();
		//var_dump($incident->incident_active); 
		//var_dump($incident->incident_verified); 
		//var_dump($incident->incident_action); 
		//var_dump($incident->$verify->verified_status); 
	 ...
	 * 
	 * @mr_k ::: se modifica en el registro para usuarios y su visualizacion
	 * 
	 */
	public static function fetch_incidents($paginate = FALSE, $items_per_page = 0)
	{
		...
		//mr_k ::: se adiciona para incident_action
		// 
		// Check if the verification status has been specified
		// 
		if (isset($url_data['aa']) AND is_array($url_data['aa']))
		{
			$verified_status = array();
			foreach ($url_data['v'] as $verified)
			{
			...
			{
				array_push(self::$params, 
					'i.incident_action IN ('.implode(",", $verified_status).')'
				);
			}
		}
