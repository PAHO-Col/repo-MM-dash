		...
		foreach ($incidents as $incident)
		{
			...
			foreach($post->data_include as $item)
			{
				switch ($item)
				{
					...
					case 3:
					$cat_titles = array();
					//var_dump($incident->incident_category);
					foreach($incident->incident_category as $category)
					{
						//mr_k ::: se adiciona la categoria padre
						$category_title = $category->category->category_title;
						$parent_id = $category->category->parent_id;
				
						// Inicializar variable para el título del padre
						$parent_title = "Sin Padre"; // Puedes cambiar esto si necesitas otro valor por defecto
				
						// Si hay un parent_id, buscar el nombre de la categoría padre
						if (!empty($parent_id)) {
							$parent_category = ORM::factory('category', $parent_id);
							if ($parent_category) {
								$parent_title = $parent_category->category_title;
							}
						}
				
						// Agregar el nombre de la categoría y su categoría padre al CSV
						$cat_titles[] = $parent_title . " - " . $category_title;
					}
					...
					if ( ! empty($custom_fields))
					{
						foreach($custom_fields as $custom_field)
						{
							...
							$field_value = $custom_field['field_response'];
							// Si field_type es 10, buscar el nombre del departamento
							if ($custom_field['field_type'] == 10) {
								$department = ORM::factory('departamento', $field_value);
								//$department = ORM::factory('departamento')->where('Id', $field_value)->find();
								if ($department->Id) { //  FIX: Verificación en Kohana 2.x
									$field_value = $department->nombre; // Cambiar el ID por el nombre
								} else {
									$field_value = "No Departamento"; // Si no encuentra el departamento
								}
							}
							if ($custom_field['field_type'] == 11) {
								$municipi = ORM::factory('municipio', $field_value);
								//$department = ORM::factory('departamento')->where('Id', $field_value)->find();
								if ($department->Id) { //  FIX: Verificación en Kohana 2.x
									$field_value = $municipi->nombre; // Cambiar el ID por el nombre
								} else {
									$field_value = "No Municipio"; // Si no encuentra el departamento
								}
							}
							$csv_line[] = $field_value;
						}
					}
					else
					{
			...
			$csv_line[] = $incident->incident_action ? 'YES' : 'NO';
