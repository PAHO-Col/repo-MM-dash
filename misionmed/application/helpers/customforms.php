...
class customforms_Core {
	...
	public static function get_custom_form_fields($incident_id = FALSE, $form_id = NULL, $data_only = FALSE, $action = "submit")
	{
		...
		//print_r("<br> SE INGRESA A LA FUNCION GET CUSTOM FORM FIELDS");
		//echo "<br> SE INGRESA A LA FUNCION GET CUSTOM FORM FIELDS";
		Kohana::log('debug', ":::::::::. HELPER (get_custom_form_fields) CUSTOMFORMS  ::");
		$fields_array = array();
	...
	public static function validate_custom_form_fields(&$post)
	{
		...
		*/
		...
		foreach ($post->custom_field  as $field_id => $field_response)
		{
			...
			// Validate Custom fields for text boxes  departamento
			if ($field_param->field_type == 10 AND isset($field_options) AND $field_response != '')
			{
				if (isset($field_options['field_datatype']))
				{
					if ($field_options['field_datatype'] == 'email' AND !valid::email($field_response))
			...
			// Validate Custom fields for text boxes  municipio
			if ($field_param->field_type == 11 AND isset($field_options) AND $field_response != '')
			{
				if (isset($field_options['field_datatype']))
				{
					if ($field_options['field_datatype'] == 'email' AND !valid::email($field_response))
	...
	public static function switcheroo($incident_id = '', $form_id = '')
	{
		...
		foreach ($disp_custom_fields as $field_id => $field_data) {
			...
			if ($field_data['field_type'] == 10) {		
					...
					$custom_forms->parents_arraydpto = $parents_arraydpto;
		...
		$custom_forms->valor_arraycampo3 = $valor_arraycampo3;

		foreach ($disp_custom_fields as $field_id => $field_data) {
			// Verificar si es un campo de tipo multiple			
			if ($field_data['field_type'] == 12) {		
					//mr_k :: se toman los select 					
					// Array para campo sexo (campo múltiple)
				$sexo_arraycampo1 = array(
				...
				$custom_forms->sexo_arraycampo1 = $sexo_arraycampo1;
				...
				$custom_forms->rangoedad_arraycampo1 = $rangoedad_arraycampo1;
			}
		}
		...
		foreach ($disp_custom_fields as $field_id => $field_data) {
			...
			if ($field_data['field_type'] == 10) {		
					...
					$custom_forms->parents_arraympio = $parents_arraympio;
	...
	public static function get_departamentos()
    {
        if (! isset(self::$manager_dpto))
		{
			...
			$manager_dpto = ORM::factory('departamento')->find_all();
	...
	public static function dpto_municipio($incident_id = '', $form_id = '',$dpto)
	{

		// Obtiene los municipios filtrados por el departamento seleccionado
		$parents_arraympio = ORM::factory('municipio')
        ->where('iddpto', $dpto)
