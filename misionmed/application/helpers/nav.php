...
class nav_Core {
	...
	public static function main_tabs($this_page = FALSE, $dontshow = FALSE)
	{
		...
		// Contacts
		/*if( ! in_array('busqueda',$dontshow))
		{
			if (Kohana::config('settings.site_contact_page') AND Kohana::config('settings.site_email') != "")
			{
				$menu_items[] = array( 
					'page' => 'busqueda',
					'url' => url::site('busqueda'),
					'name' => Kohana::lang('ui_main.busqueda')
				);	
			}
		}
*/
		// DashBoard_ en el menu
		/*if( ! in_array('dashboard',$dontshow))
		{
			if (Kohana::config('settings.site_contact_page'))
			{
				$menu_items[] = array( 
					'page' => 'dashboard',
					'url' => url::site('dashboard'),
					'name' => Kohana::lang('ui_main.dashboard')
				);	
			}
		}*/
