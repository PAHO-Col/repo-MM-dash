...
STATSCOLLECTOR;
	...
	*/
	...
	static function get_report_statsc($approved = FALSE, $by_time = FALSE, $range = NULL, $dp1 = NULL, $dp2 = NULL, $line_chart_data = FALSE, $categoriap = NULL)
	//static function get_report_stats($approved=FALSE, $by_time=FALSE, $range=NULL, $dp1=NULL, $dp2=NULL, $line_chart_data=FALSE)
	{
		if ($range === NULL)
		{
...
/*
		if ($categoriap !== NULL) 
		{
			echo "<br> CATEGORIA PADRE DIFERENTE DE NULL <br> ";
    		...
    		foreach ($incident_ids as $ic) 
			{
				...
				echo "<br>+***  incident_id_array   *****+<br> ";
				print_r($incident_id_array);
				echo "<br>**************<br> ";
				
				$reports = ORM::factory('incident')
				->where('id', 'IN', $incident_id_array)
				->where('incident_date >=', $dp1)
				->where('incident_date <=', $dp2)
				->where('incident_date >', $range_date);
				echo "<br>+***  reports   *****+<br> ";
				...
				echo "<br> CATEGORIA PADRE IGUAL  NULL <br> ";
				$reports_categories = ORM::factory('incident_category')->find_all();
				$parent_categories = ORM::factory('category')->where('parent_id', 0)->find_all(); // Obtener categorías padre
			}
*/
			...
			if ($categoriap !== NULL) {
				...
				$incident_id_array = array();
				foreach ($reports_categories as $ic) {
					$incident_id_array[] = $ic->incident_id;
				}
		...
		if ($approved)
		{
			...
			else
			{
				...
				{
					echo "<br>TERCERA ";
					echo "<br>+**** 1 VALOR DE REPORTS DEPUES DE LA CONSULTA****+<br> ";
					...
					->where('incident_date >', $range_date)
					->find_all();
					echo "<br>+**** 2 VALOR DE REPORTS DEPUES DE LA CONSULTA****+<br> ";
					print_r($reports);
					echo "<br>**************<br> ";
				}
				else {
		...
		*/
		...
		foreach ($reports as $report)
		{
			...
			$timestamp = (string) strtotime(substr($report->incident_date,0,10));
			/*if (is_string($report->incident_date)) {
				$fecha = substr($report->incident_date, 0, 10);
				$timestamp = (string) strtotime($fecha);
			} else {
				echo "Warning: incident_date no es un string para el reporte con ID " . $report->id;
				continue; // Saltamos este registro por seguridad
			}*/
