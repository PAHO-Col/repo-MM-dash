...
STATSCOLLECTOR;
	...
	*/
	...
	static function get_report_statsc($approved = FALSE, $by_time = FALSE, $range = NULL, $dp1 = NULL, $dp2 = NULL, $line_chart_data = FALSE, $categoriap = NULL, $dpto_selected = NULL)
	{
		if ($range === NULL)
		{
			$range = 100000;
		...
		if (!empty($dpto_selected)) {
			...
			if (!empty($dpto_incident_ids)) {
				...
				// Si no hay coincidencias, evita que la consulta traiga resultados vacíos
				$reports_query->where('id', -1);
			}
		}
