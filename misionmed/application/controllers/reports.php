...
class Reports_Controller extends Main_Controller {
	...
	{
		Kohana::log('debug', "YA ENTRE INDEX REPORTS ::");
		...
		// Total Reports
		//@mr_k ::: se modifican los reportes para que salgan de acuerdo a la estructura de ips y crue
		...
		// Category tree view
		//@mr_k ::: se modifican los reportes por categorias para que salgan de acuerdo a la estructura de ips y crue
		...
		Kohana::log('debug', "*********** get_custom_form_fields line 146 ::");
		$disp_custom_fields = customforms::get_custom_form_fields();
		//print_r($disp_custom_fields);
		$parents_arraydpto = array();
		$parents_arraydpto[0] = "--- Selecciona una Departamento ---";
		foreach ($disp_custom_fields as $field_id => $field_data) {
			...
			if ($field_data['field_type'] == 10) {		
					...
					$this->template->content->custom_forms_filter->parents_arraydpto = $parents_arraydpto;
		...
		foreach ($disp_custom_fields as $field_id => $field_data) {
			...
			if ($field_data['field_type'] == 11) {		
					...
					$this->template->content->custom_forms_filter->parents_arraympio = $parents_arraympio;
		...
		$this->template->content->custom_forms_filter->valor_arraycampo3 = $valor_arraycampo3;
		...
		foreach ($disp_custom_fields as $field_id => $field_data) {
			...
			if ($field_data['field_type'] == 12) {		
					...
					$this->template->content->custom_forms_filter->sexo_arraycampo1 = $sexo_arraycampo1;
				...
				$this->template->content->custom_forms_filter->rangoedad_arraycampo1 = $rangoedad_arraycampo1;
	...
	{
		//print_r("Submits a new report");
		Kohana::log('debug', "YA ENTRE Submits a new report ::");
		$db = new Database();
		...
		Kohana::log('debug', "*********** get_custom_form_fields line 321 ::");
		$form['custom_field'] = customforms::get_custom_form_fields($id,$form_id,true);

		// GET custom forms
		//print_r("<br> GET custom forms");
		$forms = array();
		...
		if ($_POST)
		{
			//print_r("<br> Check, has the form been submitted, if so, setup validation");
			...
			//	Kohana::log('debug', ":::::::: CONTROLLER::: REPORTS :::::STEP 2: SAVE INCIDENT");
		...
		//mr_k :: se toman los select de depto y municipio
		//$departamentos = ORM::factory('departamento')->get_departamentos();
		//$this->template->content->custom_forms->departamentos = $departamentos;
		//print_r($departamentos);
		//$municipio = ORM::factory('municipio')->get_municipios();
		//$this->template->content->custom_forms->municipio = $municipio;
		Kohana::log('debug', "*********** get_custom_form_fields line 421 ::");
		$disp_custom_fields = customforms::get_custom_form_fields($id, $form_id, FALSE);
		//print_r($disp_custom_fields);
		//mr_k :: se toman los select de depto y municipio si hay tipo 10
		$parents_arraydpto = array();
		
		foreach ($disp_custom_fields as $field_id => $field_data) {
			// Verificar si es un campo de tipo Departamento
			Kohana::log('debug', " *************** SE VA BUSCAR DPTO ::");
			if ($field_data['field_type'] == 10) {
				//echo "Campo Departamento encontrado: " . $field_data['field_name'] . "<br>";
				Kohana::log('debug', " *************** Campo Departamento encontrado: ::");
					//mr_k :: se toman los select de depto y municipio
					$departamentos = ORM::factory('departamento')->get_departamentos();
					$this->template->content->custom_forms->departamentos = $departamentos;
		//			print_r($departamentos);
		...
		//print_r($disp_custom_fields->field_type);
	 ...
	 * retrieved.
	 * mr_k :::  se modifica la funcion pára que solo se editen por el dueño y el usuario crue
	 */
	public function view($id = FALSE)
	{
		...
		if ($id > 0 AND Incident_Model::is_valid_incident($id,TRUE))
		{
			...
			$incident_user_valida = ORM::factory('incident')
				->select('user_id','user_id')
				->where('id',$id)
				->where('user_id',$logged_in_id)
				->find();
				...
				$logge_in_admin=Auth::instance()->get_user()->has( $superadmin_role );
			...
			$logged_crue_id = Auth::instance()->get_user()->crue_id;			

			$incident_crue_valida = ORM::factory('incident')
			->select('crue_id','crue_id')
			->where('id',$id)
			->where('crue_id',$logged_crue_id)
			->find();
			Kohana::log('debug', ":::::::::. CCCC  incident_crue_valida  CCCC ::");
			Kohana::log('debug', $incident_crue_valida->crue_id);

			////mr_k ::: se verifica que el usuario sea el propietario del registro			
			if ( ! $incident_user_valida->user_id && ! $incident_crue_valida->crue_id && Auth::instance()->get_user()->has( $superadmin_role )!=1 && Auth::instance()->get_user()->has( $admin_role )!=5) 	{				
		...
		Kohana::log('debug', "*********** get_custom_form_fields line 795 ::");
		$form_field_names = customforms::get_custom_form_fields($id, $incident->form_id, FALSE, "view");
		//mr_k ::: se adiciona para el cargue
		$parents_arraydpto = array();
		$parents_arraydpto[0] = "--- Selecciona una Departamento ---";
		foreach ($form_field_names as $field_id => $field_data) {
			// Verificar si es un campo de tipo Departamento
		...
		$disp_custom_fields = customforms::get_custom_form_fields($id, $incident->form_id, FALSE);
	...
	public function geocode()
	{
		$this->template = "";
		$this->auto_render = FALSE;
		//mr_k direccion que se busca $_POST['address']
    ...
    */
	...
	/**
	 * Ajax MUNICIPIO call to update Incident Reporting Form
	 */
	public function tomar_mpio()
	{
		Kohana::log('debug', "*************** TOMAR_MPIO ::");
		$this->template = "";
		$this->auto_render = FALSE;
		isset($_POST['form_id']) ? $form_id = $_POST['form_id'] : $form_id = "1";
		isset($_POST['incident_id']) ? $incident_id = $_POST['incident_id'] : $incident_id = "";
		isset($_POST['dpto']) ? $dpto = $_POST['dpto'] : $dpto = "";
		
		$form_fields = customforms::dpto_municipio($incident_id,$form_id,$dpto);
		echo json_encode(array("status"=>"success", "response"=>$form_fields));
	}
	public function latomar_mpio()
{
		...
		isset($_POST['incident_id']) ? $incident_id = $_POST['incident_id'] : $incident_id = "";
		isset($_POST['dpto']) ? $dpto = $_POST['dpto'] : $dpto = "";
		Kohana::log('debug', "form_id: $form_id, dpto: $dpto, incident_id: $incident_id");
		...
		public function latomar_mpio_crue()
{
		...
		//$form_fields = customforms::dpto_municipio_crue($dpto);
		
		$form_fields = Stats_Model::dpto_municipio_crue($dpto);
		echo json_encode(array("status"=>"success", "response"=>$form_fields));
