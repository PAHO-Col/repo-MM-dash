...
/********************************************************************************** */		
		...
		if ($user_role_info['is_ips']) {
		    ...
		    . 'WHERE i.incident_active = 1 '.$incident_id_in.' AND i.user_id = '.$user_role_info['user_id'].' '		   
		...
		if ($user_role_info['is_crue']) {
			...
			$query = 'SELECT UNIX_TIMESTAMP('.$select_date_text.') AS time, COUNT(i.id) AS number '
		    . 'FROM '.$this->table_prefix.'incident AS i '						
		    . 'WHERE i.incident_active = 1 '.$incident_id_in.' AND i.crue_id = '.$user_role_info['crue_id'].' '		   
		    . 'GROUP BY '.$groupby_date_text;
		}
		//if ($this->auth->get_user()->has( $superadmin_role )==1)
...
/*********************************************************************************** */		
		Kohana::log('debug',":::: CCC  consulta sql (controlador json) :::: ".$query);
	...
	{
		Kohana::log('debug',":::: CCC  Read in new layer KML via HttpClient  (controlador json) :::: ");
		$this->template = "";
		$this->auto_render = FALSE;
	...
	{
		Kohana::log('debug',":::: CCC  Get Geometry JSON (controlador json) :::: ");
		$geometry = array();
		if ($incident_id)
		{
