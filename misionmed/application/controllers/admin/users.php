...
class Users_Controller extends Admin_Controller {
	...
	public function index()
	{
		...
		//para el select de ip
		$parents_arrayips = array();

		//mr_k:: se adiciona la variable de get
		$role_id = $this->input->get('role_id');
		$tipo = $this->input->get('tipo');
    	$id_ips_crue = $this->input->get('id_ips_crue');
		//Kohana::log('debug', "++++++++++++++++++++ Valor de role_id: " . $role_id);
		//print_r("++++++++++++++++++++ Valor de role_id: " . $id_ips_crue);
		...
		$user_role_info = $incident_model->get_user_role_and_association();	
			//	  print_r("++++++++++++++++++++ Valor de user_role_info: " );
		...
		if ($user_role_info['is_crue']) {
			//echo "AKI FUE....".$user_role_info['is_crue'];
			$parents_arrayips = ORM::factory('ips')->where('crue_id', $user_role_info['crue_id'])->select_list('Id', 'nombre_ips');
			$parents_arraycrue = ORM::factory('crue')->where('Id', $user_role_info['crue_id'])							
							->select_list('Id', 'Nombre');
		
		}
		else{
			//echo "AKI FUE";
			$parents_arrayips = ORM::factory('ips')->where('crue_id', $id_ips_crue)							
							->select_list('Id', 'nombre_ips');
							$parents_arraycrue = ORM::factory('crue')							
							->select_list('Id', 'Nombre');
		}
		//print_r($parents_arraycrue);
		...
		$parents_arraycrue = ORM::factory('crue')							
							->select_list('Id', 'Nombre');
		*/					
		//$parents_arraycrue[0] = "--- Selecciona un Departamento ---";
		//ksort($parents_arraycrue);
		...
		$select_roles = ORM::factory('role')
						->where('name != "Administrador" and name != "superadmin" and name != "login"')
						->orderby('name', 'asc')->find_all();				
		...
		if ($user_role_info['is_crue']) {	
			//echo "<br>VALOR CRUE ID... ANTES DEL IF....".$crue_id;		
			$users_query = ORM::factory('user')        	
			->join('crues_ips','users.crue_id','crues_ips.crue_id')
			->where('crues_ips.crue_id', $crue_id);			
		}
		else {
		...
		//mr_k ::: se valida id rol para cambiar la consulta sql con respecto al id
		//echo "<br>VALOR role_id ... ANTES DEL IF....".$role_id;
		if ($role_id != "") {
			//$users_query->join('roles_users', 'INNER')
			...
			//->where('roles_users.user_id', $role_id);	

			$users_query->join('roles_users', 'roles_users.user_id', 'users.id')
			->where('roles_users.role_id', $role_id);

			//SELECT u.* FROM users as u,roles_users as ru WHERE ru.role_id = 7 AND ru.user_id = u.Id
		}
		

				// 🔹 Verificar consulta antes de ejecutarla
		//		Kohana::log('debug', "Consulta final SQL: " . $users_query->sql());
		//print_r("<br>valor final de la consulta");
		//print_r($users_query);

		
		//mr_k ::: select rol search user 
		$role_array[0] = "--- Filtrar Usuarios por ROL ---"; 
		
		foreach ($select_roles as $role)
		{
			// Cambiar la clave para mantener el ID como clave y el nombre como valor
			$role_array[$role->id] = $role->name; // Cambié de $role_array[$role->id] = $role->id; a $role_array[$role->id] = $role->name
		}/**/
		// Add one additional role for users with no role
		$role_array['none'] = utf8::strtoupper(Kohana::lang('ui_main.none'));

		//print_r("<br>ANTES DE LA FUNCION...".$tipo);
		/*if ($tipo != "" && $id_ips_crue != "") {
			print_r("<br>ENTRE EN LA FUNCION".$id_ips_crue );
			//$users_query->where('users.crue_id', '=', $id_ips_crue);
			$users_query = ORM::factory('user')
			->where('crue_id', $id_ips_crue)
						->orderby('name', 'asc');
		...
		$this->template->content->id_ips_crue=$id_ips_crue;
		$this->template->content->tipo=$tipo;
	...
	public function roles()
	{
		...
		if ($_POST)
		{
			...
			if ($post->action == 'a')// Add / Edit Action
			{
				...
				$post->add_rules('name', 'required', 'length[3,100]', 'alpha_numeric');
	...
	@mr_k	::: funcion para el modulo de usuarios 
	*/
	...
	*/
	...
	public function filter_by_role()
	{
		...
		Kohana::log('debug', "+++++++++++++++++++++ Valor de post->action: " . $role_id);
    
    $users_query = ORM::factory('user');

    if ($role_id && $role_id != 'none') {
        $users_query->join('roles_users', 'users.id', 'roles_users.user_id')
                    ->where('roles_users.role_id', $role_id);
    }
	$role_array[0] = "--- Filtrar Usuarios por ROL ---"; 
		$roles = ORM::factory('role')->where('name != "Administrador" and name != "superadmin" and name != "login"')->orderby('name', 'asc')->find_all();
		foreach ($roles as $role)
		{
			// Cambiar la clave para mantener el ID como clave y el nombre como valor
			$role_array[$role->id] = $role->name; // Cambié de $role_array[$role->id] = $role->id; a $role_array[$role->id] = $role->name
...
//	Kohana::log('debug', "++++++++++++++++++++++++ Valor de post->action: " . $users);

    // Cargar solo la vista con la lista de usuarios
    ...
    //echo $view->render();
    //exit; // Finalizar para evitar que se mezcle con otras salidas del controlador

	}

	/**
	 * mr_koyote ::: funcion para listar ips de acuerdo a un crue
	 * 
	 * 
	 */
		...
		{
			$crue_id = $this->input->get('crue_id');

			if ($crue_id) {
				$ips = ORM::factory('ips')->where('crue_id', $crue_id)->find_all();
				
				$ips_array = [];
				foreach ($ips as $ip) {
					$ips_array[] = [
						'id' => $ip->Id,
						'nombre' => $ip->nombre_ips
					];
				}
				// Enviar respuesta JSON y salir del script para evitar problemas con la vista
				header('Content-Type: application/json');
				echo json_encode($ips_array);
				exit();
				} else {
				echo json_encode([]);
				exit();
