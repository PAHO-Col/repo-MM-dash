	...
	public function index()
	{
		...
		if($role_id!="")
		{
			if ($role_id && $role_id != 'none') {
				...
				Kohana::log('debug', ":::::::::. VALOR DEL ROL A CONSULTAR " . $role_id);
				//echo $users_query->compile(Database::instance());
				var_dump($users_query);
				}				
				print_r("<br>PRIMERA CONSULTA SQL users_query<br>");
				//print_r($users_query);	
					
				
		}
		else{
			Kohana::log('debug', ":::::::::. ENTRE EN ELSE ");			
				if ($_POST)
				{
					$post = Validation::factory(array_merge($_POST, $_FILES));
					var_dump($post);
					//$elvalor_post=$post;
					//Kohana::log('debug', ":::::::::. Valor de post->action: " . $elvalor_post);
					...
					//mr_koyote
					//print_r("<br>VALOR VARIABLE CUANDO SE ENVIA EL FORMULARIO ");
					//Kohana::log('debug', ":::::::::. Valor de post->action: " . $post->action);
		...
		*/
		...
		if ($user_role_info['is_crue']) {			
			$users_query = ORM::factory('user')        	
			->join('crues_ips','users.crue_id','crues_ips.Id')
			->where('crues_ips.crue_id', $crue_id);	
			print_r("<br>SEGUNDA CONSULTA SQL users_query<br>");
			//print_r($users_query);				
		}
		else {
			...
			if (! $this->auth->get_user()->has( $superadmin_role ) )
			{
				...
				$users_query
					...
					print_r("<br>TERCERA CONSULTA SQL users_query<br>");
				//	print_r($users_query);		
			}
		...
		if($role_id!="")
		{
            ...
            ->where('roles_users.role_id', '=', $role_id)
            ->select('users.*'); // Aseguramos seleccionar solo los datos de users
			$users = $users_query->find_all((int)Kohana::config('settings.items_per_page_admin'), $pagination->sql_offset);
			print_r("<br>CONSULTA SQL GENERADA:<br>");
			print_r($sql_query);
			var_dump($users_query);
			...
			$users_query->join('roles_users', 'users.id', 'roles_users.user_id')
			->where('roles_users.role_id', $role_id);
			print_r("<br>ULTIMA VVVVVVVVV CONSULTA SQL users_query PERO SEE ASGINA A USERS<br>");
			$users = $users_query->find_all((int)Kohana::config('settings.items_per_page_admin'), $pagination->sql_offset);
			Kohana::log('debug', ":::::::::. VALOR users = users_query ");	
			//Kohana::log('debug', ":::::::::. VALOR CONSULTA SQL ".$users);	
			print_r("<br>ULTIMA VVVVVVVVV CONSULTA SQL users_query PERO SEE ASGINA A USERS<br>");
			var_dump($users);
			*/
			...
			$users = $users_query->find_all((int)Kohana::config('settings.items_per_page_admin'), $pagination->sql_offset);
			print_r("<br>ULTIMA ZZZZZZZZZZZZZ CONSULTA SQL users_query PERO SEE ASGINA A USERS<br>");
			Kohana::log('debug', ":::::::::. TODA LA CONSULTA  ");			


		}
		//comentariado tmp
		//$users = $users_query->find_all((int)Kohana::config('settings.items_per_page_admin'), $pagination->sql_offset);
		print_r("<br>ULTIMA XXXXX CONSULTA SQL users_query PERO SEE ASGINA A USERS<br>");
		print_r($users);		

		//mr_k ::: creada si se envio un id rol
		//$role_id = $this->input->post('role_id');
		/*print_r("<br>VALOR ROLE_ID<br>");
			print_r($role_id);		
		if ($role_id != ""){
			...
			->where('roles_users.role_id', $role_id);	
			print_r("<br>ULTIMA  SSSS CON POST CONSULTA SQL users_query PERO SEE ASGINA A USERS<br>");
			print_r($users_query);		
