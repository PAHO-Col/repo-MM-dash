...
class Reports_Controller extends Admin_Controller {
	...
	public function index($page = 1)
	{
		...
		if ( !empty($_GET['status']))
		{
			...
			elseif (strtolower($status) == 'aa')
			{
				array_push($this->params, 'i.incident_action = 0');
			}
		...
		if ($_POST)
		{
			...
			$post->add_rules('action','required', 'alpha', 'length[1,2]');
			...
			//mr_k :: adicionar el permiso en la tabla para llevar la estructura  AccionIncidente aa
			if ($post->action == 'aa' AND ! Auth::instance()->has_permission('reports_acciones'))
			{
				$post->add_error('action','permission');
			}
			...
			////print_r("<br>VALOR DEL POST 1::".$post->action);
			if ($post->action == 'a')
			{
			...
			if ($post->validate())
			{
			//	//print_r("<br>VALOR DEL POST 3::".$post->action);
				// Approve Action
				if ($post->action == 'a')
				{
				...
				{
					//print_r("<br>VALOR DEL POST 4::".$post->action);
					foreach ($post->incident_id as $item)
					{
						$update = new Incident_Model($item);
						if ($update->loaded == TRUE)
				...
				{
					//print_r("<br>VALOR DEL POST 5::".$post->action);
					foreach ($post->incident_id as $item)
					{
						$update = new Incident_Model($item);
				...
				// Verify Action ActionIncidente   aa				
				elseif ($post->action == 'aa')
				{
					
					////print_r("<br>VALOR DEL POST 6::".$post->action);
					////print_r("ENTRE EN EL VALOR AA");
					Kohana::log('debug', ":::::::::. VALOR DE AA ::");
					...
					foreach ($post->incident_id as $item)
					{
						...
						if ($update->loaded == TRUE)
						{
							if ($update->incident_action == '1')
							{
								$update->incident_action = '0';
								$verify->verified_status = '0';
							}
							else
							{
								$update->incident_action = '1';
								$verify->verified_status = '9';
							}
							$update->save();
					...
					// Set the form action
					$form_action = utf8::strtoupper(Kohana::lang('ui_admin.accion_reporte'));
				}
				
				// Delete Action
				elseif ($post->action == 'd')
				{
					////print_r("<br>VALOR DEL POST 7::".$post->action);
					foreach ($post->incident_id as $item)
					{
						$update = new Incident_Model($item);
						if ($update->loaded)
			...
			////print_r("<br>VALOR DEL POST 8::".$post->action);
		}
	 ...
	 * @mr_k ::: se adicionara validacion de usuario y el registro,
	 */
	public function edit($id = FALSE, $saved = FALSE)
	{
		////print_r("INICIE EL EDIT<br>");
		$db = new Database();
		
		// If user doesn't have access, redirect to dashboard
		if ( ! $this->auth->has_permission("reports_edit"))
		{	
			//@mr_k ::: se cambia la ruta de reenvio
			//url::redirect('admin/dashboard');
			url::redirect('admin/dashboard');
		}
		//mr_k ::: se adiciona validacion que el editor sea el dueño del registro.
		//mr_k ::: se crea una nueva consulta verifico que sea el usuario dueño del registro
		...
		if ($resultado_ips_count > 0)
			{
				$incident_user_valida = ORM::factory('incident')->select('user_id','user_id')->where('id',$id)->where('user_id',$logged_in_id)->find();
			}
			if ($resultado_crue_count > 0)
			{				
				$incident_user_valida = ORM::factory('incident')->select('user_id','user_id')->where('id',$id)->where('crue_id',$crue_in_id)->find();
			}
			if ($admin_in  > 0)
			{				
				$incident_user_valida = ORM::factory('incident')->select('user_id','user_id')->where('id',$id)->find();
			}

			////echo "<br>VALOR ::::".$incident_user_valida->user_id.":";
			if ( ! $incident_user_valida->user_id) 
			{	if($admin_in == 0)
				{
				url::redirect('admin/dashboard');
				}				
		...
		////print_r("<br>ANTES DEL DEL IF POST<br>".$_POST);
		// Check, has the form been submitted, if so, setup validation
		if ($_POST)
		{
		//	//print_r("<br>*** DENTRO DEL DEL IF POST<br>");
			...
			//echo "<br>SE IMPRIME EL VALOR DE POST<br>";
			//print_r($post);
			...
			if (reports::validate($post))
			{
				...
				////print_r("FORMULARIO	HEY YA VOY A GUARDAR!!!!");
		...
		//mr_k ::: se adiciona get departamentos
		$parents_arraydpto = array();
		$parents_arraydpto[0] = "--- Selecciona un Departamento ---";
		foreach ($disp_custom_fields as $field_id => $field_data) {
			// Verificar si es un campo de tipo Departamento
			
			if ($field_data['field_type'] == 10) {		
					//mr_k :: se toman los select de depto y municipio					
					$parents_arraydpto = ORM::factory('departamento')->select_list('Id', 'nombre');
					$parents_arraydpto[0] = "--- Selecciona un Departamento ---";
					ksort($parents_arraydpto);
					$this->template->content->custom_forms->parents_arraydpto = $parents_arraydpto;

			}
		}
		$parents_arraympio = array();		
		foreach ($disp_custom_fields as $field_id => $field_data) {
			// Verificar si es un campo de tipo Departamento			
			if ($field_data['field_type'] == 11) {		
					//mr_k :: se toman los select de depto y municipio					
					$parents_arraympio = ORM::factory('municipio')->select_list('Id', 'nombre');
					$parents_arraympio[0] = "--- Selecciona un Municipio ---";
					ksort($parents_arraympio);
					$this->template->content->custom_forms->parents_arraympio = $parents_arraympio;

			}
		}

		$sexo_arraycampo1 = array();
		$rangoedad_arraycampo1 = array();
		$valor_arraycampo3 = "";
		//$valor_arraycampo3 = 'placeholder="Ingrese valor"';
		$this->template->content->custom_forms->valor_arraycampo3 = $valor_arraycampo3;
		...
		foreach ($disp_custom_fields as $field_id => $field_data) {
			...
			if ($field_data['field_type'] == 12) {		
					...
					ksort($sexo_arraycampo1);
					////print_r($sexo_arraycampo1);					
					$this->template->content->custom_forms->sexo_arraycampo1 = $sexo_arraycampo1;

					// Array para campo rango etáreo
				$rangoedad_arraycampo1 = array(
				...
				ksort($rangoedad_arraycampo1);
				
				$this->template->content->custom_forms->rangoedad_arraycampo1 = $rangoedad_arraycampo1;
	...
	public function download()
	{
		...
		if ($_POST)
		{
			...
			if (download::validate($post))
			{
				...
				/*
				if (in_array(1, $post->data_accion))
				{
					$show_accion = TRUE;
				}

				if (in_array(0, $post->data_accion))
				{
					$show_no_accion = TRUE;
				}*/
				...
				$filter .= ' AND ';
				/*
				if ($show_accion && !$show_no_accion)
				{
					$filter .= ' incident_action = 1 ';
				}
				elseif (!$show_accion && $show_no_accion)
				{
					$filter .= '  incident_action = 0 ';
				}
				elseif ($show_accion && $show_no_accion)
				{
					$filter .= ' (incident_action = 1 OR incident_action = 0) ';
				}
				...
				{
					// Equivalent to 1 = 0
					$filter .= ' (incident_action = 0 AND incident_action = 1) ';
				}
				...
				*/
				...
				//Limpiar los enter en el campo descripcion de los incidentes y la tabla form_response
				
				Database::instance()->query("UPDATE incident SET incident_description=REPLACE(incident_description,'\n','')");
				Database::instance()->query("UPDATE form_response SET form_response=REPLACE(form_response,'\n','')");
				//UPDATE incident SET incident_description = REPLACE(incident_description,"\n","")
				//UPDATE form_response SET form_response = REPLACE(form_response,"\n","")
				
				
				
				// Retrieve reports
				$incidents = ORM::factory('incident')->where($filter)->orderby('incident_dateadd', 'desc')->find_all();
				//print_r("<br> filtro final: ");
				//var_dump($incidents);
				//exit;
	...
	//@mr_k esta funcion fue modificada para que el cargue pueda actualizar o crear.
	public function upload()
	{
	//	//print_r("<br>funcion upload");
		$form = array(
			'uploadfile' => '',
		);
		...
		{
			////print_r("<br>funcion upload:::: POST");
			$post = array_merge($_POST, $_FILES);
			...
			if($post->validate(TRUE))
				...
				if (file_exists($_FILES['uploadfile']['tmp_name']))
					...
					if (in_array($extension, $allowable_extensions))
					{
						...
						//pasamos a la libreria 00:26:20: File /media/koyote/disko/SINERGIA/USHA/2020/SINERGIA/mount_www/application/libraries/CSVImporter.php opened(2).
	...
	*/
	...
	/**
	 * mr_koyote::: se modifica la funcion de busqueda para poder buscar elementos de los formulario 
	...
	private function _search_form()
	{
		...
		if (! $location_filter)
		{
			if ( isset($_GET['start_loc']) )
			{
				...
				////print_r("<br>SE INGRESO EN _search_form<br>");
			}
		...
		//mr_k::: inicio creando la consulto de los campos para el select
		$select_campos = array();		
		$select_campos = ORM::factory('form_field')->select_list('id', 'field_name');
		$select_campos[0] = "--- Selecciona un Campo ---";
		//
		$parents_arraydpto = array();
		$parents_arraydpto[0] = "--- Selecciona un Departamento ---";
		//mr_k :: se toman los select de depto y municipio					
		...
		// Documento
		if (!isset($_GET['documento']) OR !is_array($_GET['documento']))
		{
			$_GET['documento'] = isset($_GET['documento']) ? array($_GET['documento']) : array();
		}
		$search_form->documento = $_GET['documento'];
		...
		// mr_k :180022025:: Obtener el número de documento si está presente en la búsqueda
		$document_number = isset($_GET['document']) ? trim($_GET['document']) : '';
		$search_form->document = $document_number;
		$search_form->documento = $_GET['documento'];
		...
		//mr_k::: se envio el de campos
		$search_form->select_campos = $select_campos;
		$search_form->parents_arraydpto = $parents_arraydpto;
