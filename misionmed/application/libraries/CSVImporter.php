...
class CSVImporter {	
			...
			// Loop through CSV rows
		 	foreach($rows as $row)
		 	{
				...
				if (isset($row['#']) AND isset($this->incident_ids[$row['#']]))
				{
					...
					$this->import_report_u($row);
					$this->notices[] = Kohana::lang('import.incident_exists').$row['#'];
				}
				else
				{
					
					//se manda a la fucion import report
	...
	function import_report($row)
	{
		...
		//@mrkoyote  ******	//finalizan las validaciones
		
		
		// STEP 1: SAVE LOCATION
		//@mr_koyote komentariado para actualizar
		...
		//@mr_koyote
		//se manda el valor que trae el id en la carga del archivo
		$incident->id = isset($row['#']) ? $row['#'] : '';	
		//
		...
		$incident->crue_id = $row['CRUEID'];
		Kohana::log('debug',"CRUE ID");
		Kohana::log('debug',$incident->incident_title);
		$incident->save();
	...
	function import_report_u($row)
	{
		
		// If the date is not in proper date format
		//@mr_koyote se realizan las validaciones.
		if (!strtotime($row['INCIDENT DATE']))
		{
			$this->errors[] = Kohana::lang('import.incident_date').($this->rownumber+1).': '.$row['INCIDENT DATE'];
		}
