...
# --- Definición de opciones para filtros (Después de cargar la base) ---
opciones_departamento <- c("Todos", sort(unique(base$departamento)))
opciones_categoria    <- c("Todas", sort(unique(catalogo_categorias$categoria_principal)))
opciones_afectacion   <- c("Todos", sort(unique(base$afectacion)))
opciones_autor        <- c("Todos", sort(unique(base$tipo_autor)))
opciones_estado       <- c("Todos", sort(unique(base$estado_caso)))
...
#------------------------------------------------------------------
# Inicialización de filtros (se ejecuta una sola vez)
#------------------------------------------------------------------
observeEvent(TRUE, {
...
#------------------------------------------------------------------
# Días de oportunidad válidos
#------------------------------------------------------------------
dias_oportunidad_validos <- reactive({

  datos_filtrados()$dias_oportunidad %>%
    na.omit() %>%
    .[. >= 0]
...
#------------------------------------------------------------------
# KPI - Promedio días de reporte
#------------------------------------------------------------------
output$kpi_promedio_reporte <- renderValueBox({

  d <- dias_oportunidad_validos()
...
output$kpi_24h <- renderValueBox({
  ...
  valueBox(
    ...
    if (length(d) == 0)
      ...
      paste0(round(mean(d <= 1) * 100, 1), "%"),

    "Reportados en 24 horas",
...
#------------------------------------------------------------------
hay_datos <- function(datos, variable){

  nrow(datos) > 0 &&
    variable %in% names(datos)
...
# Gráfico vacío
#------------------------------------------------------------------
grafico_vacio <- function(
    texto = "Sin datos para los filtros seleccionados"
){
  ...
  graf_barras <- function(datos, variable, etiqueta = "", top_n = 30, alto_margen = 260, ylab = "Número de casos", paleta = c("#D7ECFF", "#005B96"), colores_fijos = NULL) {
    if (!hay_datos(datos, variable)) {

		  return(grafico_vacio())
