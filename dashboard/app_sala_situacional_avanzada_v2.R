...
leer_base <- function() {
  if (file.exists("mm2026mayo.xlsx")) {
    ...
    stop("No se encontro la base. Guarde mm2026mayo.xlsx o mm2026mayo.csv en la misma carpeta del archivo app_sala_situacional_avanzada.R")
  }
  as.data.frame(base)
}
...
col_fecha        <- "INCIDENT DATE"       # Columna D
col_categoria    <- "CATEGORY"           # Columna G
col_departamento <- "Departamento-1"      # Columna S
col_municipio    <- "Municipio-1"         # Columna T
col_afectacion   <- "Afectación específica-1"
col_autor        <- "Tipo de autor-1"
col_estado       <- "Estado del caso-1"
col_institucion  <- "Tipo de Institución-1"
col_lat          <- "LATITUDE"
col_lon          <- "LONGITUDE"
